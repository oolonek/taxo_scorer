...
input_cleaned_ISDB_path <- "../Data/Curated/ISDB_curated.tsv"

#Path to cleaned annotation result of MSFinder
input_cleaned_MSFinder_path <- "../Data/Curated/MSFinder_curated.tsv"

#Path to cleaned annotation result of Sirius
input_cleaned_Sirius_path <- "../Data/Curated/Sirius_curated.tsv"
```

###Outputs
```{r}
#Path to weighted annotation result of ISDB
output_weighted_ISDB_path <- "../Data/Weighted/ISDB_weighted.tsv"

#Path toweighted annotation result of MSFinder
output_weighted_MSFinder_path <- "../Data/Weighted/MSFinder_weighted.tsv"

#Path to weighted annotation result of Sirius
output_weighted_Sirius_path <- "../Data/Weighted/Sirius_weighted.tsv"
...
```{r}
ISDBout_tax_STDS <- read_feather(input_cleaned_ISDB_path)

MSFinderout_tax_STDS <- read_feather(input_cleaned_MSFinder_path)

Siriusout_tax_STDS <- read_feather(input_cleaned_Sirius_path)
```

### Loading the function
```{r}
source("../Taxo_Functions/TaxoWeighting_Filtering_Plotting_functions.R")
```

###Applying the function
```{r}
ISDBout_unweighted <- TaxoWeighter(0, 0, 0, ISDBout_tax_STDS)
MSFinderout_unweighted <- TaxoWeighter(0, 0, 0, MSFinderout_tax_STDS)
Siriusout_unweighted <- TaxoWeighter(0, 0, 0, Siriusout_tax_STDS)

ISDBout_weighted <- TaxoWeighter(1, 2, 3, ISDBout_tax_STDS)
MSFinderout_weighted <- TaxoWeighter(1, 2, 3, MSFinderout_tax_STDS)
Siriusout_weighted <- TaxoWeighter(1, 2, 3, Siriusout_tax_STDS)
```
...
#Exporting
```{r}
write_feather(Siriusout_weighted, output_weighted_Sirius_path)
write_feather(MSFinderout_weighted, output_weighted_MSFinder_path)
write_feather(ISDBout_weighted, output_weighted_ISDB_path)
write_feather(F1_scores, output_f1_scores_path)
```