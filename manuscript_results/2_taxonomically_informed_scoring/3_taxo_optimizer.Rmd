...
# Load functions
```{r}
TaxoWeighingTool_01 <- function(score_kin, score_phy, score_cla, score_ord, score_fam, score_gen, score_spe)
  {
...
```{r}
TaxoWeighingTool_02 <- function(score_kin, score_phy, score_cla, score_ord, score_fam, score_gen, score_spe)
  {
...
```{r}
TaxoWeighingTool_03 <- function(score_kin, score_phy, score_cla, score_ord, score_fam, score_gen, score_spe)
  {
...
```{r}
TaxoWeighingTool_04 <- function(score_kin, score_phy, score_cla, score_ord, score_fam, score_gen, score_spe)
  {
...
```{r}
TaxoWeighingTool_05 <- function(score_kin, score_phy, score_cla, score_ord, score_fam, score_gen, score_spe)
  {
...
```{r}
TaxoWeighingTool_06 <- function(score_kin, score_phy, score_cla, score_ord, score_fam, score_gen, score_spe)
  {
...
```{r}
TaxoWeighingTool_07 <- function(score_kin, score_phy, score_cla, score_ord, score_fam, score_gen, score_spe)
  {
...
```{r}
TaxoWeighingTool_08 <- function(score_kin, score_phy, score_cla, score_ord, score_fam, score_gen, score_spe)
  {
...
```{r}
Results_degraded_MC_01 <- MonteCarlo(
   func =  TaxoWeighingTool_01,
   nrep = 10,
   param_list = bounds,
   ncpus = 10
   )
...
```{r}
df_test <- as.data.frame(Results_degraded_01$BestPars)
```
...
```{r}
ISDBout_weighted_new <- TaxoWeighter(1, 2, 3, 4, 5, 6, 7, ISDBout_tax_STDS_degraded_01)

ISDBout_weighted_old <- TaxoWeighter_old(1, 2, 3, ISDBout_tax_STDS_degraded_01)
```

```{r}
TP_new <- ISDBout_weighted_new %>% 
   filter(RANK_FINAL == 1)

TP_old <- ISDBout_weighted_old %>% 
   filter(RANK_FINAL == 1)
```

```{r}
Inc_new <- ISDBout_weighted_new %>% 
   filter(RANK_FINAL <= RANK_BEFORE)

Inc_old <- ISDBout_weighted_old %>% 
   filter(RANK_FINAL <= RANK_BEFORE)
```

```{r}
Dec_new <- ISDBout_weighted %>% 
   filter(RANK_FINAL > RANK_BEFORE)

Dec_old <- ISDBout_weighted_old %>% 
   filter(RANK_FINAL > RANK_BEFORE)
```